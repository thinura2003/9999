...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'love ?(.*)', fromMe: true, deleteCommand: false,  desc: dd }, (async (message, match) => {

        if (match[1] === '') return await message.sendMessage(Lang.NEED_WORD);

        var ttinullimage = await axios.get(`https://videfikri.com/api/textmaker/mugflower/?text=${match[1]}`, { responseType: 'arraybuffer' })
    ...
    Asena.addCommand({ pattern: 'love ?(.*)', fromMe: false,deleteCommand: false, desc: dd}, (async (message, match) => {

...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'zwood ?(.*)', fromMe: true, deleteCommand: false,desc: Lang.WOOD_DESC }, (async (message, match) => {

        if (match[1] === '') return await message.sendMessage(Lang.NEED_WORD);

        const ttinullimagex = await await got(`https://ttpxc.herokuapp.com/api/textmaker?text=${match[1].replace(/Ã–/g, "%C3%96").replace(/Ã¶/g, "%C3%B6").replace(/Ã¼/g, "%C3%BC").replace(/Ãœ/g, "%C3%9C").replace(/Äž/g, "%C4%9E").replace(/ÄŸ/g, "%C4%9F").replace(/ÅŸ/g, "%C5%9F").replace(/Åž/g, "%C5%9E").replace(/Ã§/g, "%C3%A7").replace(/Ã‡/g, "%C3%87").replace(/Ä±/g, "%C4%B1").replace(/i/g, "%69").replace(/"/g, "%22").replace(/Ä°/g, "%C4%B0")}`, { responseType: 'arraybuffer' }).then(async ok  => {
    ...
    Asena.addCommand({ pattern: 'zwood ?(.*)', fromMe: false, deleteCommand: false,desc: Lang.WOOD_DESC }, (async (message, match) => {
